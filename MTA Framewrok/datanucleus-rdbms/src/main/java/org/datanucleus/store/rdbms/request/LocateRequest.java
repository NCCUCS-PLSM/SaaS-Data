...
public class LocateRequest extends Request
{
    ...
    public void execute(ObjectProvider op)
    {
        if (statementLocked != null)
        {
            ...
            short lockType = ec.getLockManager().getLockMode(op.getObjectId());
                    ...
                    try
                    {
                        ...
                        else if (cmd.getIdentityType() == IdentityType.APPLICATION)
                        {
                            op.provideFields(cmd.getPKMemberPositions(),
                                storeMgr.getFieldManagerForStatementGeneration(op, ps, mappingDefinition, false));
                        }
            ...
            {
                String msg = LOCALISER.msg("052220", op.toPrintableID(), statement, sqle.getMessage());
