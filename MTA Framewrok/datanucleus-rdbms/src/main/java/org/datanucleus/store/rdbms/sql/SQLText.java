...
public class SQLText
{
                ...
                if (mapping != null)
                {
                    ...
                    if (param.getColumnNumber() >= 0)
                    {
                        ...
                        else
                        {
                            ...
                            MappedStoreManager storeMgr = mapping.getStoreManager();
    ...
    //todo arthur add
    public com.arthur.mta.utbdbservice.sql.WhereClause toUTbDBService()
    {
    		AbstractSQLFactory df = AbstractSQLFactory.getFactory(SQLFactoryTypes.DataNucleus);
    	   StringBuffer sql = new StringBuffer();
           com.arthur.mta.utbdbservice.sql.WhereClause node = null;
           
           if(this.isUTbWhereClauseComposite()){
        	   node =  df.createWhereClauseComposite();
           }
           ...
           for (int i = 0; i < appended.size(); i++)
           {
               ...
               if (item instanceof SQLExpression)
               {
            	   ...
            	   if(item instanceof BooleanExpression){
            		   node.add(st.toUTbDBService());
            	   }else{
            		   if(st.toSQL().equals("?")){
            			   List<SQLStatementParameter> lps = st.getParametersForStatement();
            			   for (SQLStatementParameter sqlStatementParameter : lps) {     			
            				   sql.append(sqlStatementParameter.getValue());
            				   break;
            			   }
            		   }else{
            			   sql.append(st.toSQL());
               ...
               {
            	   String operator = item.toString().toLowerCase().trim();
            	   if(operator.equals("and") || operator.equals("or")){
            		   node.addLogicOperator(item.toString());
            	   }else if(operator.equals("(")){
            		  
            	   }else if(operator.equals(")")){
            		   node.getChild(node.getLastChildIndex()).barcketed(true);
           ...
           if( sql.toString() != null && sql.toString().length() != 0){
        	   node = df.createWhereClause(sql.toString());
           } 
   ...
   //todo arthur add
    private boolean isUTbWhereClauseComposite(){
