      ...
      <div class="span3">
        ...
        <div class="span9">
			...
			if(TenantName!=null)
			{
				...
				String FieldType = "";
				String FieldLength = "";
				FieldName = new String(request.getParameter("fieldname").getBytes("ISO-8859-1"));
				NewFieldName = new String(request.getParameter("newfieldname").getBytes("ISO-8859-1"));
				ResultSet schemas = (ResultSet) meta.getColumns("", "", "CourseInfo","",TenantName);
				while(schemas.next())
				{
					if(schemas.getString(4).equals(FieldName))
					{
						FieldType = schemas.getString(6);
						if(FieldType.equals("CHAR"))
						{
							FieldLength = schemas.getString(7);
						}
					}
				}
				String altersql = "";
				if(FieldType.equals("CHAR"))
				{
					altersql = "ALTER TABLE CourseInfo change "+FieldName+" "+NewFieldName+" "+FieldType+"("+FieldLength+")";
				}
				else
				{
					altersql = "ALTER TABLE CourseInfo change "+FieldName+" "+NewFieldName+" "+FieldType;
				}

				((Multitenantstatement) stmt).settenantId(TenantName);
				//out.print(altersql+FieldLength );
