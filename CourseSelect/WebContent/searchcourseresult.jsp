	  ...
	  <div class="span3">
          <div class="well sidebar-nav">
            <ul class="nav nav-list">
              ...
              <li class="active"><a href="searchcourse.jsp?TenantName=<%= TenantName%>">Search Course</a></li>
            </ul>
            <ul class="nav nav-list">
        ...
        <div class="span9">
		  ...
		  <form>
	        ...
	        String CourseName = new String(request.getParameter("CourseName").getBytes("ISO-8859-1"));
	        String TeacherName = new String(request.getParameter("TeacherName").getBytes("ISO-8859-1"));
	        String CourseId = new String(request.getParameter("CourseId").getBytes("ISO-8859-1"));
	        String Days = new String(request.getParameter("Days").getBytes("ISO-8859-1"));
	        //out.print(Days);
			String wherecondition = "";
	        if(!(CourseName.equals("")))
	        {
	        	wherecondition = wherecondition + " AND CourseName LIKE '%"+CourseName+"%'";
	        }
	        if(!(TeacherName.equals("")))
	        {
	        	wherecondition = wherecondition + " AND Instructors LIKE '%"+TeacherName+"%'";
	        }

	        if(!(CourseId.equals("")))
	        {
	        	wherecondition = wherecondition + " AND CourseId = '%"+ CourseId+"%'";
	        }
	        if(!(Days.equals("Days")))
	        {
	        	wherecondition = wherecondition + " AND Days = '"+Days+"'";
	        }
			...
			stmt.close();
			//out.print(TotalPage+" "+NowPage+"-");
			if(TotalPage==0)
			{
				out.print("<h2>No such record is found!!</h2>");
			}
			else
			{
				...
				sql = "SELECT CourseId, CourseName, Instructors,Days, Time FROM CourseInfo WHERE IsClosed = 0";
				if(!(wherecondition.equals("")))
		    	{
		    		sql = sql +wherecondition;
		    	}
				sql = sql + " LIMIT 6 OFFSET "+Integer.toString(OffsetValue);
				((Multitenantstatement) stmt).settenantId(TenantName);
